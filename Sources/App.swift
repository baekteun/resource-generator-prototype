...
struct GenerateCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        // Fonts 생성
        if let fonts = config.fonts {
            for input in fonts.inputs {
                for output in fonts.outputs {
                    try generateFonts(input: input, output: output)
                }
            }
        }
    }
    ...
    private func generateFonts(input: String, output: OutputConfig) throws {
        var command = FontsCommand()
        command.path = input
        command.output = output.output
        command.templatePath = output.templatePath
        try command.run()
...
// MARK: - Base Command
protocol ResourceGeneratorCommand: ParsableCommand {
    var path: String { get }
    var output: String { get }
    var templatePath: String? { get set }
    
    func generateCode() throws
}

extension ResourceGeneratorCommand {
    func validate() throws {
        let inputPath = Path(path)
        guard inputPath.exists else {
            throw ValidationError("입력 경로가 존재하지 않습니다: \(path)")
        }
    }
...
// MARK: - Assets Command
struct AssetsCommand: ResourceGeneratorCommand {
    static let configuration = CommandConfiguration(
        commandName: "assets",
    ...
    var output: String = "Assets+Generated.swift"
...
struct StringsCommand: ResourceGeneratorCommand {
    static let configuration = CommandConfiguration(
        commandName: "strings",
    ...
    @Option(name: .shortAndLong, help: "생성된 코드가 저장될 경로입니다.")
    var output: String = "Strings+Generated.swift"
...
// MARK: - Fonts Command
struct FontsCommand: ResourceGeneratorCommand {
    static let configuration = CommandConfiguration(
        commandName: "fonts",
        abstract: "지정된 경로의 폰트 파일들을 분석하여 Swift 코드를 생성합니다."
    )
    
    @Argument(help: "폰트 파일들이 위치한 경로입니다.")
    var path: String
    
    @Option(name: .shortAndLong, help: "생성된 코드가 저장될 경로입니다.")
    var output: String = "Fonts+Generated.swift"
    ...
    func generateCode() throws {
        ...
        if let customTemplatePath = templatePath {
            ...
            let templatePath = Path(Bundle.module.path(forResource: "Fonts", ofType: "stencil", inDirectory: "Templates")!)
            templateString = try String(contentsOf: templatePath.url)
        }
        
        // TODO: 폰트 파일 파싱 및 코드 생성 로직 구현
        print("폰트 파일 분석이 완료되었습니다.")
    }
}
